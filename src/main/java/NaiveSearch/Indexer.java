...
/**
 * Indexer job, which combines in itself three jobs: Word enumerator, IDF and TFIDF.
 */
public class Indexer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Configuration TFIDFJobConf = new Configuration();
        ...
        // Word enumerator job initialization
        ...
        // Configuring and launching the word enumerator job
        ...
        // Initialization and configuration of Inverse Document Frequency job
        ...
        // TF/IDF job
        Job TFIDFJob = Job.getInstance(TFIDFJobConf, "TFIDF Job");
        FileInputFormat.addInputPath(TFIDFJob, new Path(outIDF));
        FileOutputFormat.setOutputPath(TFIDFJob, new Path(outIndexer));
        if (fs.exists(new Path(outIndexer))) {
            fs.delete(new Path(outIndexer), true);
        }

        TFIDFJob.setMapOutputKeyClass(IntWritable.class);
        TFIDFJob.setMapOutputValueClass(Text.class);
        TFIDFJob.setOutputKeyClass(IntWritable.class);
        TFIDFJob.setOutputValueClass(Text.class);

        TFIDFJob.setJarByClass(TFIDF.class);
        TFIDFJob.setMapperClass(TFIDF.MapJob.class);
        TFIDFJob.setReducerClass(TFIDF.ReduceJob.class);
